...
char *gBindAddr    = NULL,              // local interface to bind to NULL
     *gBindPort    = "4567";            // local port to bind to 5150
...
void HandleIo(ULONG_PTR key, BUFFER_OBJ *buf, HANDLE CompPort, DWORD BytesTransfered, DWORD error)
{
    ...
    if (buf->operation == OP_ACCEPT)
    {
		...
		// 打印客户端连接ip和端口
		char *szip = inet_ntoa(((sockaddr_in*)LocalSockaddr)->sin_addr);
		int  uport = ntohs(((sockaddr_in*)LocalSockaddr)->sin_port);

		char *szip1 = inet_ntoa(((sockaddr_in*)RemoteSockaddr)->sin_addr);
		int  uport1 = ntohs(((sockaddr_in*)RemoteSockaddr)->sin_port);
		printf("socket: %d local: %s:%u remote: %s:%u\n",buf->sclient,
			szip, uport, szip1, uport1);
        ...
        if (clientobj)
        {
			// 设置客户端sock端口重用，保证本地连接与宽带连接不同地址可以使用同一端口
			int nOpt = 1;
			int errorCode = setsockopt(buf->sclient, SOL_SOCKET, SO_REUSEADDR, (char*)&nOpt, sizeof(nOpt));
			if(SOCKET_ERROR == errorCode)
			{
				printf("socket SO_REUSEADDR mode error");
				return ;
			}
    ...
    else if (buf->operation == OP_READ)
    {
        ...
        if (BytesTransfered > 0)
        {
            ...
            EnqueuePendingOperation(&gPendingSendList, &gPendingSendListEnd, sendobj, OP_WRITE); // 将接收数据添加到发送链表
        }
        else
        {
    ...
    ProcessPendingOperations(); // 将客户端接收数据转发
...
int /*__cdecl*/ main(int argc, char **argv)
{
    ...
    while (ptr)
    {
		...
		// 设置监听sock端口重用，保证本地连接与宽带连接不同地址可以使用同一端口
		int nOpt = 1;
		int errorCode = setsockopt(listenobj->s, SOL_SOCKET, SO_REUSEADDR, (char*)&nOpt, sizeof(nOpt));
		if(SOCKET_ERROR == errorCode)
		{
			printf("socket SO_REUSEADDR mode error");
			return -1;
